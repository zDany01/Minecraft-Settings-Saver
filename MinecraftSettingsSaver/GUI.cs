...
namespace MinecraftSettingsSaver
{
    public partial class GUI : Form
    {
        ...
        public GUI(string exportedProfilesFilePath)
        {
            ...
            this.Load += GUI_Load;
            importProfilesDialog.FileOk += ImportProfilesDialog_FileOk;
            ...
            saveProfilesDialog.FileOk += SaveProfilesDialog_FileOk;
        ...
        private void SaveProfileBtn_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(profileNameTxbx.Text))
            {
            ...
            {
                if (profileNameTxbx.Text.Contains(item))
                {
                    MessageBox.Show("Insert a valid name for the profile.", APP_NAME, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            profileNameTxbx.Text = RemoveFinalSpaces(profileNameTxbx.Text);
            #endregion
            ...
            string fileName = ApplicationDataDir + profileNameTxbx.Text + ".smc";
            ...
            using (ZipArchive archive = new ZipArchive(File.OpenWrite(fileName), ZipArchiveMode.Create))
            {
                ...
                {
                    writer.WriteLine($"{profileNameTxbx.Text}\n{minecraftVersionCBox.SelectedItem}\n{optifineSettingsCBox.Checked}");
                }
            }
            profileNameTxbx.Clear();
            RefreshProfilesList();
        }


        private void SaveProfilesDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            {
                using (StreamWriter writer = new StreamWriter(archive.CreateEntry("info").Open())) writer.Write(INFO_MESSAGE);
            }
        }
        ...
        private void ProfilesListBox_SelectedValueChanged(object sender, EventArgs e) {
            ...
            if (profilesListBox.SelectedIndex == 0) profilesListBox.SelectedIndex = -1;
        }
    }
}
